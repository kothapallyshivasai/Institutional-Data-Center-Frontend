...
export default function StudentHome() {
    ...
    useEffect(() => {
        async function getSkills() {
            try {
                const url = "http://127.0.0.1:9000/skill/get-all-skills-by-id";
        ...
        async function getAchievements() {
            try {
                ...
                const updatedAchievements = await Promise.all(response.data.map(async (achievement) => { 
                    try {
                        const response = await axios.get("http://127.0.0.1:9000" + achievement.certify, {
                            headers: {
                                'Authorization': 'Bearer ' + jwtToken,
                            },
        ...
        async function getInternships() {
            try {
                ...
                const updatedInternships = await Promise.all(response.data.map(async (internship) => { 
                    try {
                        const response = await axios.get("http://127.0.0.1:9000" + internship.verification, {
                            headers: {
                                'Authorization': 'Bearer ' + jwtToken,
                            },
                            responseType: 'arraybuffer'
    ...
    async function uploadSkill(e){
        ...
        try {
            const response = await axios.post(
              'http://127.0.0.1:9000/skill/add-skill',
              {
    ...
    async function uploadAchievement(e) {
        ...
        try {
            const response = await axios.post(
                'http://127.0.0.1:9000/certification/add-certification',
                formData,
                {
                    headers: {
    ...
    async function uploadProject(e){
        ...
        try {
            const response = await axios.post(
              'http://127.0.0.1:9000/project/add-project',
              {
    ...
    async function getStudentImage(imageUrl) {
        try {
            ...
            const blob = new Blob([response.data], { type: response.headers['content-type'] });
            setStudent(prevStudent => ({ ...prevStudent, profilePicture: URL.createObjectURL(blob) }));
        } catch (error) {
    ...
    async function uploadImage(event) {
        ...
        try {
            const response = await axios.post("http://127.0.0.1:9000/student/set-student-image", formData, {
                headers: {
                    'Content-Type': 'multipart/form-data',
                    'Authorization': 'Bearer ' + jwtToken,
                }
                    ...
                    <div className="col-xl-6 col-lg-8 offset-lg-2 offset-xl-3">
                            ...
                            <div className="card-body">
                                <div className="row mt-3">
                                    <div className="col-sm-2 col-12 justify-content-sm-end align-items-sm-top d-flex">
                                        {!profilePictureError ? (
                                            <img src={student.profilePicture} alt="Profile" className='img-fluid img-student-profile' />
                                        ) : (
                                            <img src={upload_image_logo} onClick={() => document.getElementById('fileInput').click()} alt="Upload" className='img-fluid img-upload-file' />
                                        )}
                                        <input id="fileInput" type="file" accept="image/*" className='img-upload-file-input' onChange={uploadImage} />
                                    </div>
                                    <div className="col-sm-10 mt-3 mt-sm-0">
                                        <h3>{student.studentName}</h3>
                                        <div className="mt-2 text-muted">
                                            <b>{username} | B.Tech, {student.department} | {student.batch}</b>
                                        </div>
                                        <div className="mt-2 text-muted">
                                        ...
                                        <div className="row mt-2">
                                            <div className="col-lg-3 col-sm-6 col-12 text-muted">
                                                <img src={telephone_logo} alt="telephone_logo" className='img-fluid icons-home-page' /> {student.mobileNumber}
                                            </div>
                                            <div className="col-lg-6 col-sm-6 col-12 text-muted">
                                                <img src={email_logo} alt="email_logo" className='img-fluid icons-home-page' /> {student.emailId}
                                            </div>
                                        </div>
                                        <div className="row mt-4">
                                            <div className="col-12">
                                                <img src={linkedin_student_logo} alt="linkedin_student_logo" className='img-fluid linkedin-logo-student' />
                                                <img src={github_student_logo} alt="github_student_logo" className='img-fluid github-logo-student' />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div className="card-body">
                                <div className="row"> 
                                    <div className="col-12">
                                        ...
                                        Talk about the achievements you have completed, What projects you undertook and what special skills you learned.
                                        <button type="button" className="bi btn bi-caret-down-fill float-end" data-bs-toggle="collapse" data-bs-target="#achievementsDropdown" aria-expanded="false" aria-controls="achievementsDropdown">NT</button> 
                                        <button type="button" className="bi btn bi-caret-down-fill float-end" data-bs-toggle="collapse" data-bs-target="#achievementsDropdown2" aria-expanded="false" aria-controls="achievementsDropdown2">T</button> 
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="card mt-4 shadow"> 
                            <div className="card-body">
                                <div className="row">
                                    <div className="col-12">
                                        <button type="button" className="bi btn bi-plus-lg float-end" data-bs-toggle="modal" data-bs-target="#skillBackdrop"></button>
                                        <h5><b>Skills</b></h5>
                                        Talk about your skills that made you proud and contributed to your practical learnings.
                                        <button type="button" className="bi btn bi-caret-down-fill float-end" data-bs-toggle="collapse" data-bs-target="#skillsDropdown" aria-expanded="false" aria-controls="skillsDropdown"></button> 
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="card mt-4 shadow"> 
                            <div className="card-body">
                                <div className="row">
                                    <div className="col-12">
                                        <button type="button" className="bi btn bi-plus-lg float-end" data-bs-toggle="modal" data-bs-target="#internshipBackdrop"></button>
                                        <h5><b>Internships</b></h5>
                                        Talk about your internships that made you proud and contributed to your learnings.
                                        <button type="button" className="bi btn bi-caret-down-fill float-end" data-bs-toggle="collapse" data-bs-target="#internshipsDropdown" aria-expanded="false" aria-controls="internshipsDropdown"></button> 
                                    </div>
                                </div>
                            </div>
                        </div>
