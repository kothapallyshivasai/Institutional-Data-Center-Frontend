...
export default function StudentHome() {
  ...
  useEffect(() => {
    ...
    async function getAchievements() {
      try {
        const url =
          "http://127.0.0.1:9000/certification/get-all-certifications-by-id";
        const response = await axios.get(url, {
          headers: {
            "Content-Type": "application/json",
            Authorization: "Bearer " + jwtToken,
          ...
          response.data.map(async (achievement) => {
            try {
              const response = await axios.get(
                "http://127.0.0.1:9000" + achievement.certify,
                {
                  headers: {
                    Authorization: "Bearer " + jwtToken,
                  },
    ...
    async function getInternships() {
      try {
        const url =
          "http://127.0.0.1:9000/internship/get-all-internships-by-id";
        const response = await axios.get(url, {
          headers: {
            "Content-Type": "application/json",
            Authorization: "Bearer " + jwtToken,
        ...
        const updatedInternships = await Promise.all(
          response.data.map(async (internship) => {
            try {
              const response = await axios.get(
                "http://127.0.0.1:9000" + internship.verification,
                {
                  headers: {
                    Authorization: "Bearer " + jwtToken,
                  },
  ...
  async function getStudentObject() {
    try {
      const url =
        "http://127.0.0.1:9000/student/get-student/" + jwtDecode(jwtToken).sub;
      const response = await axios.get(url, {
        headers: {
          "Content-Type": "application/json",
          Authorization: "Bearer " + jwtToken,
  ...
  async function uploadImage(event) {
    ...
    try {
      const response = await axios.post(
        "http://127.0.0.1:9000/student/set-student-image",
        formData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
