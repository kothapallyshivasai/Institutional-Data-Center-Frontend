...
export default function FacultyAdminHome() {
    ...
    useEffect(() => {
        ...
        async function getFacultyData() {
            try {
                ...
                const url = "http://127.0.0.1:9000/faculty/get-all-faculties";
                ...
                const facultyWithImages = await Promise.all(response.data.map(async faculty => {
                    const imageData = await getFacultyImage(faculty.profilePicture);
                    if (imageData != null){
                        return { ...faculty, profilePicture: URL.createObjectURL(imageData)};
                    }
                    return faculty;
                }));
                ...
                const facultyWithExperiences = await Promise.all(facultyWithImages.map(async faculty => {
                    if (facultyIds.includes(faculty.facultyId)) {
                        const experienceData = await getExperiencesByFaculty(jwtToken, faculty.facultyId);
                ...
                const facultyWithPapers = await Promise.all(facultyWithExperiences.map(async faculty => {
                ...
                const facultyWithCertifications = await Promise.all(facultyWithPapers.map(async faculty => {
                    if (facultyIds3.includes(faculty.facultyId)) {
                        const certifyData = await getCertificationsByFaculty(jwtToken, faculty.facultyId);
    ...
    useEffect(() => {
        ...
        // department
        if (departmentChoice) {
            filteredFaculty = filteredFaculty.filter(faculty => faculty.department === departmentChoice);
        }

        // designation
        if (designationChoice) {
            filteredFaculty = filteredFaculty.filter(faculty => faculty.designation === designationChoice);
        }
            ...
            <div className="body-part pb-5 mt-3">
                <div className="row pt-4">
                    <div className="col-xl-3 col-lg-4">
                        <FacultyFilterBar setDepartmentChoice={setDepartmentChoice} setDesignationChoice={setDesignationChoice} setExperienceChoice={setExperienceChoice}
                                            uniqueDepartments={uniqueDepartments} uniqueDesignations={uniqueDesignations} uniqueExperiences={uniqueExperiences} />
                    </div>
                    <div className="col-xl-7 col-lg-8 offset-xl-1">
                        <FacultyPageSelection modifiedfaculty={modifiedFaculty} itemsPerPage={itemsPerPage} downloadFaculty={downloadFaculty} jwtToken={jwtToken} setItemsPerPage={setItemsPerPage} />
                        {modifiedFaculty.length === 0 ? (
                            ...
                            <>
                                ...
                                <FacultyPagination currentPage={currentPage} handlePageChange={handlePageChange} pageNumbers={pageNumbers} />
                            </>
                        )}
                    </div>
                </div>
...
async function getUniqueDepartments(logoutUser, jwtToken){
    try {
        const url = "http://127.0.0.1:9000/faculty/get-unique-departments";
        const response = await axios.get(url, {
            headers: {
                'Content-Type': 'application/json',
                'Authorization': 'Bearer ' + jwtToken
...
async function getUniqueDesignations(logoutUser, jwtToken){
    try {
        const url = "http://127.0.0.1:9000/faculty/get-unique-designations";
        const response = await axios.get(url, {
            headers: {
                'Content-Type': 'application/json',
                'Authorization': 'Bearer ' + jwtToken
...
async function getUniqueExperiences(logoutUser, jwtToken){
    try {
        const url = "http://127.0.0.1:9000/experience/get-unique-experiences";
        const response = await axios.get(url, {
            headers: {
                'Content-Type': 'application/json',
                'Authorization': 'Bearer ' + jwtToken
...
async function downloadFaculty(jwtToken) {
    try {
        const url = "http://127.0.0.1:9000/faculty/excel";
...
async function getUniqueFacultyIdsByExperience(jwtToken){
    try{
        const url = "http://127.0.0.1:9000/experience/get-all-unique-faculty-ids-by-experience";
        const response = await axios.get(url, {
            headers: {
                'Content-Type': 'application/json',
                'Authorization': 'Bearer ' + jwtToken
...
async function getExperiencesByFaculty(jwtToken, facultyId){
    try{
        const url = "http://127.0.0.1:9000/experience/get-unique-experiences-by-faculty-id/" + facultyId;
        const response = await axios.get(url, {
            headers: {
                'Content-Type': 'application/json',
                'Authorization': 'Bearer ' + jwtToken
...
async function getUniqueFacultyIdsByPaper(jwtToken){
    try{
        const url = "http://127.0.0.1:9000/research-papers/get-all-unique-faculty-ids-by-paper";
        const response = await axios.get(url, {
            headers: {
                'Content-Type': 'application/json',
                'Authorization': 'Bearer ' + jwtToken
...
async function getPapersByFaculty(jwtToken, facultyId){
    try{
        const url = "http://127.0.0.1:9000/research-papers/get-unique-papers-by-faculty-id/" + facultyId;
        const response = await axios.get(url, {
            headers: {
                'Content-Type': 'application/json',
                'Authorization': 'Bearer ' + jwtToken
...
async function getUniqueFacultyIdsByCertification(jwtToken){
    try{
        const url = "http://127.0.0.1:9000/faculty-certification/get-all-unique-faculty-ids-by-certification";
        const response = await axios.get(url, {
            headers: {
                'Content-Type': 'application/json',
                'Authorization': 'Bearer ' + jwtToken
...
async function getCertificationsByFaculty(jwtToken, facultyId){
    try{
        const url = "http://127.0.0.1:9000/faculty-certification/get-unique-certifications-by-faculty-id/" + facultyId;
        const response = await axios.get(url, {
            headers: {
                'Content-Type': 'application/json',
                'Authorization': 'Bearer ' + jwtToken
